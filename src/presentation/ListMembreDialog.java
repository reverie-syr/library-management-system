...
@Getter
@Setter
public class ListMembreDialog extends JDialog {
    ...
    // TableModeleAuteur
    private final TableModeleMembre tableModeleMembre = TableModeleMembre.getInstance();

    // JTable
    private final JTable table = new JTable(tableModeleMembre);
    ...
    public ListMembreDialog(JPanel parentPanel){
        super(adminParentFrame, "Liste Des Membres", true);
        ...
        tableModeleMembre.chargerTable(librairie.getAllMembre());
        ...
        table.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 1) {
                    ...
                    membre = librairie.getMembre(id);

                    if (parentPanel instanceof AttribuerLivre){
                        final AttribuerLivre parentPanel = AttribuerLivre.getInstance();

                        parentPanel.getMembreIdSpinner().setValue(membre.getId());
                        parentPanel.getMemberNameLabel().setText(String.format("%s %s", membre.getNom(), membre.getPrenom()));
                    }

                    ListMembreDialog.this.dispose();
                }
            }
        });
    }
